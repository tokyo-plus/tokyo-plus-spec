...
|---|---|---|---|---|
| `INT-04 電車・運行情報` | `よく乗る路線` | 公共交通オープンデータセンターから取得した路線IDと表示名 | 0件以上を複数選択。未選択なら対象路線を広く扱う | 路線IDのみを端末内に保存し、必要な判断要求時だけ一時的に利用する。自宅駅、勤務先駅、乗車経路、利用履歴は取得しない。 |
...
| S-03 電車の運行情報通知 | `通勤`、`外出`、`旅行` | `電車・運行情報`、子タグ`よく乗る路線`（任意） | 公共交通オープンデータセンターの運行情報。選択した路線を優先し、未選択時は対象データ範囲の運行情報を確認して、余裕を持つことを提案する。 |
...
タグ、通知設定をAIエージェントに渡し、AIエージェントが必要なオープンデータ・外部APIを探索して通知候補を作成する。データソースの対象範囲はデータソースレジストリと取得アダプターが管理する。ただし、AIエージェントが任意のURLをそのまま判断根拠や通知に利用することは認めず、許可されたカタログAPI、取得アダプター、鮮度・ライセンス検証を経たデータだけを利用する。
...
| 2. データ探索 | AIエージェント＋データソースレジストリ | タグ、子タグ、データソースの対象範囲に関連するシナリオパック、東京都オープンデータ、外部APIの候補を選ぶ。未承認のソースは候補から除外する。 |
| 3. 取得・正規化 | 取得アダプター | 選ばれたソースを取得し、鮮度、対象範囲、ライセンス、出典を確認して正規化する。 |
| 4. 通知候補作成 | AIエージェント | 正規化データから推奨行動1件、理由、不確実性、参照データを含む通知候補を作る。 |
| 5. 最終通知判定 | ルールエンジン | 基準値、タグ・子タグ・データスコープとの関連性、データ鮮度、通知間隔、1日の上限、重複、通知権限を検証し、通知・保存・破棄を決める。 |

AIエージェントは通知候補を作成できるが、通知の最終判定、通知上限の変更、重複通知の抑止、危険性の高い判断の確定は行わない。
...
| Workers AI | タグ・子タグ・通知設定と正規化済みデータから、データ探索計画と通知候補を構造化生成 |
...
2. AIエージェントがデータソースレジストリと許可されたカタログAPIを探索し、関連するシナリオパックとデータソース候補を選ぶ。
3. データ取得アダプターが候補ソースからデータを取得する。
4. 取得データを共通の正規化形式へ変換し、データソース名、取得時刻、有効期限、出典URLを付与する。天気を必須とし、未検証の候補データは除外する。
5. AIエージェントが正規化データから通知候補を作成する。
6. JSON Schema検証、禁止値・必須項目検証、文字数検証を行う。
7. ルールエンジンが基準値、タグ・エリアとの関連性、鮮度、通知設定、重複を検証し、最終的な通知可否を判定する。
...
| F-02 | 判断生成 | ○ | タグ、子タグ、通知設定、正規化済みデータからAIエージェントが通知候補を生成する |
...
AIエージェントは通知候補を作成するが、最終的な通知判定はルールエンジンが行う。判断生成・通知判定は、次の条件を満たす場合に実行する。

- 天気データが取得できている。
- 補助データを使う場合は、データソースごとの鮮度、対象範囲、ライセンス確認が完了している。
- 対象範囲に重要な変化がある。
- 変化が利用者コンテキストタグ、関心タグ、子タグ、またはデータソースの対象範囲と関連する。
...
AIエージェントの責務は、利用者コンテキストタグ、関心タグ、子タグ、通知設定をもとに、許可されたデータソースから必要なデータを探索・選択し、取得アダプターが正規化したデータから通知候補を生成することである。通知候補には、利用者が取るべき行動、理由、不確実性、参照データを含める。
...
| `data_scope` | TEXT | データソース側の対象範囲。利用者設定やAI入力には含めない |
| `title` | TEXT | 判断タイトル |
...
利用者に地域を選択させる項目は設けず、利用者向けの地域コードもAI入力には含めない。データソースの対象範囲は、取得アダプターとシナリオパックの設定として管理する。MVPでは、気象・地震情報は東京都および周辺、路上工事は23区、公衆トイレは中央区、電車の運行情報はODPTで提供される対象事業者・路線を対象とする。デモで特定地点のレコードを使う場合も、利用者設定ではなく検証用の固定条件として扱う。
...
| 条件付き採用 | 公共交通オープンデータセンター `odpt:TrainInformation` API | [TrainInformation API](https://api.odpt.org/api/v4/odpt:TrainInformation) / [開発者サイト](https://developer.odpt.org/) | 事業者、路線、運行情報、発生時刻、データ生成時刻、有効期限 | ユーザー登録と`consumerKey`が必要。対象事業者・路線、実レスポンス、利用条件を確認するまで通知判断には使わない。 |
...
| S-03 | 電車の運行情報通知 | 通勤、外出、旅行 | 公共交通オープンデータセンター、気象庁 | 対象路線に遅延、運休、ダイヤ乱れ等の公式情報がある | 出発前に公式運行情報を確認し、時間に余裕を持つ | 条件付き採用、API取得待ち |
...
2026-08-08（日本時間）に、共通ソース3件を実取得してMVPの入力形式に正規化した。さらに、路上工事、地震情報の各データも個別に実取得し、シナリオパックの発火条件と出力例を確認した。電車の運行情報は、公共交通オープンデータセンターのAPIをデータソースとして選定したが、利用登録と`consumerKey`が必要なため、実レスポンスの取得・発火検証は未完了である。
...
同じ共通出力形式で、路上工事・地震情報の追加シナリオが実データから成立することを確認した。電車の運行情報通知は、データソースと入出力仕様を定義し、API取得後に実データ検証を行う。

#### 電車の運行情報通知シナリオ

公共交通オープンデータセンターの`odpt:TrainInformation` APIを利用し、対象データ範囲に含まれる路線の遅延、運休、ダイヤ乱れ等を検出する。`contextTags=["通勤"]`、`interestTags=["電車・運行情報"]`、`interestChildTags=["よく乗る路線:選択された路線ID"]`の場合、選択路線を優先して出発前に公式運行情報を確認する通知候補を作成する。
...
| 条件付き採用 | 公共交通オープンデータセンター `odpt:TrainInformation` API | 無料のユーザー登録で利用できるが、APIキーが必要。運行情報の対象事業者・路線、利用条件、実レスポンスを確認後にS-03を実装済みとする。 |
...
| API-04 | 公共交通オープンデータセンター `odpt:TrainInformation` API | [TrainInformation API](https://api.odpt.org/api/v4/odpt:TrainInformation) / [開発者サイト](https://developer.odpt.org/) | 要ユーザー登録・`consumerKey` | 事業者、路線、運行情報、発生時刻、データ生成時刻、有効期限 | 条件付き採用。ユーザー登録は無料だが、実レスポンス取得と対象路線の確認は未完了。 |
...
API-04の取得形式は次のとおりとする。`consumerKey`はサーバー側のSecretとして管理し、アプリには返却しない。
...
| `scopeCode` | `tokyo`、`23ku`、`chuo`等のデータ対象範囲コード |
...
- 公共交通運行情報: `dc:date`（データ生成時刻）と`dct:valid`（有効期限）を記録し、有効期限を超えた情報は判断に使わない。画面にはデータ生成時刻と出典を表示する。対象事業者・路線の提供範囲が確認できない場合は通知候補から除外する。
...
- 公共交通オープンデータセンターの利用登録、対象事業者・路線、APIレスポンス、利用条件を確認し、S-03を実装済みへ移行する。
