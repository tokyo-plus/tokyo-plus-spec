...
タグは、利用者の関心・利用状況に沿った判断通知を提供するための、固定語彙による設定値である。タグそのものを個人属性の収集やプロファイリングに利用せず、利用エリア、通知設定、採用済みデータソースと組み合わせて、AIエージェントが判断に必要なシナリオとデータを選ぶために利用する。

タグは次の2種類に分ける。

| ID | 種類 | 表示名 | 定義 |
|---|---|---|---|
| CTX-01 | 利用者コンテキストタグ | `通勤` | 定期的な移動の場面を表す。勤務先や職業は収集しない。 |
| CTX-02 | 利用者コンテキストタグ | `屋外移動` | 屋外を移動する場面を表す。 |
| CTX-03 | 利用者コンテキストタグ | `徒歩` | 徒歩で移動する場面を表す。 |
| CTX-04 | 利用者コンテキストタグ | `自転車` | 自転車で移動する場面を表す。 |
| CTX-05 | 利用者コンテキストタグ | `外出` | 日常の外出を表す。 |
| CTX-06 | 利用者コンテキストタグ | `テレワーク` | 外出先で作業する場面を表す。勤務先や業務内容は収集しない。 |
| CTX-07 | 利用者コンテキストタグ | `旅行` | 東京を訪問・回遊する場面を表す。居住地や旅行履歴は収集しない。 |
| INT-01 | 関心タグ | `暑さ・熱中症対策` | 暑さへの備えに関する通知を表す。健康状態は収集しない。 |
| INT-02 | 関心タグ | `雨・天候変化` | 雨や天候変化への備えに関する通知を表す。 |
| INT-03 | 関心タグ | `休憩・トイレ` | 休憩・トイレ候補の確認に関する通知を表す。 |
| INT-04 | 関心タグ | `通信・FREE Wi-Fi` | 外出先の通信手段の確認に関する通知を表す。 |
| INT-05 | 関心タグ | `交通・道路工事` | 道路工事・経路確認に関する通知を表す。 |
| INT-06 | 関心タグ | `防災・地震情報` | 公式防災情報の確認に関する通知を表す。 |
...
| S-03 外出先の通信確保 | `外出`、`テレワーク`、`旅行` | `通信・FREE Wi-Fi` | 東京都FREE Wi-Fi。経路上のアクセスポイント候補の確認を提案する。 |
...
- `利用エリア`はタグではなく、位置情報の許可を得た場合に一時的に生成するエリアコードとして扱う。居住地、勤務先、学校等をタグとして登録しない。
...
上記のほか、タグ単体または匿名ID・利用エリアとの組み合わせにより、特定の個人を識別・推測できる内容は追加しない。イベントなど新しい関心タグを追加する場合も、対応するデータの地域範囲、鮮度、利用条件を確認してから選択肢に追加する。

### 2.4 AIエージェントとルールエンジンの分担

タグ、利用エリア、通知設定をAIエージェントに渡し、AIエージェントが必要なオープンデータ・外部APIを探索して通知候補を作成する。ただし、AIエージェントが任意のURLをそのまま判断根拠や通知に利用することは認めず、データソースレジストリ、許可されたカタログAPI、取得アダプター、鮮度・ライセンス検証を経たデータだけを利用する。

処理の責務は次のとおりとする。

| 段階 | 担当 | 処理 |
|---|---|---|
| 1. コンテキスト受領 | Workers API | 利用者コンテキストタグ、関心タグ、利用エリア、通知設定を検証してAIエージェントへ渡す。匿名ID、APNsトークン、正確な緯度・経度は渡さない。 |
| 2. データ探索 | AIエージェント＋データソースレジストリ | タグと利用エリアに関連するシナリオパック、東京都オープンデータ、外部APIの候補を選ぶ。未承認のソースは候補から除外する。 |
| 3. 取得・正規化 | 取得アダプター | 選ばれたソースを取得し、鮮度、対象エリア、ライセンス、出典を確認して正規化する。 |
| 4. 通知候補作成 | AIエージェント | 正規化データから推奨行動1件、理由、不確実性、参照データを含む通知候補を作る。 |
| 5. 最終通知判定 | ルールエンジン | 基準値、タグ・エリアとの関連性、データ鮮度、通知間隔、1日の上限、重複、通知権限を検証し、通知・保存・破棄を決める。 |
...
| Workers API | 匿名ID検証、設定・履歴API、タグ・利用エリア・通知設定の受領、AIエージェントの実行管理、データ取得、ルールエンジン、通知連携 |
...
| Workers AI | タグ・利用エリア・通知設定と正規化済みデータから、データ探索計画と通知候補を構造化生成 |
...
1. Cron Triggersが設定された間隔で、対象利用者のタグ・利用エリア・通知設定を使う判断ジョブを起動する。
...
| F-02 | 判断生成 | ○ | タグ、利用エリア、通知設定、正規化済みデータからAIエージェントが通知候補を生成する |
...
- 対象エリアに重要な変化がある。
- 変化が利用者コンテキストタグ、関心タグ、または利用エリアと関連する。
...
AIエージェントの責務は、利用者コンテキストタグ、関心タグ、利用エリア、通知設定をもとに、許可されたデータソースから必要なデータを探索・選択し、取得アダプターが正規化したデータから通知候補を生成することである。通知候補には、利用者が取るべき行動、理由、不確実性、参照データを含める。
...
東京都FREE Wi-Fi CSVから、東京駅周辺の「公衆電話（千代田区丸の内1-6）」等のアクセスポイントを抽出できた。`contextTags=["外出","テレワーク"]`、`interestTags=["通信・FREE Wi-Fi"]`、`targetArea="tokyo_station"`の場合、次のような判断を生成できる。
...
AIエージェントによる「探索」は、まずこのカタログAPIとデータソースレジストリから候補を見つける処理を指す。候補を判断に利用するには、取得アダプターの許可リストへの登録、形式・ライセンス・鮮度・対象エリアの検証、シナリオパックとの対応付けを完了しなければならない。
...
初回起動時は、利用者コンテキストタグと関心タグの目的・利用範囲を説明したうえで選択させる。タグは複数選択とし、自由入力欄は設けない。選択したタグは匿名ID単位の設定として保存し、次回以降のAIエージェントによるデータ探索と通知候補作成に利用する。タグ未選択で進んだ場合は、天気を必須とする一般的な判断に限定する。
...
- タグに居住地、勤務先、学校、年齢、性別、家族構成、健康状態、正確な予定、移動履歴を含めない。利用エリアはタグと分離し、位置情報の許可時に一時的なエリアコードとして利用する。
