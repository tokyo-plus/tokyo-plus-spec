...
flowchart TD
  Onboarding[初回タグ選択]
  InitialConfirmation[初回利用設定確認]
  Home[ホーム]
  HomeExpanded[ホーム / 詳細展開]
  Menu[サイドメニュー]
  Settings[設定]
  History[履歴]
  Onboarding --> InitialConfirmation --> Home
  Home --> HomeExpanded
  Home --> Menu
  Menu --> Settings
  Menu --> History
...
#### 8.1.0 共通レイアウト・コンポーネントルール

初回設定を含む主要画面では、Apple寄りの余白感を優先し、画面左右の余白は16px、セクション同士の間隔は24px、Cardの内側余白は16pxを基準にする。見出しと内容の間隔は8pxから12pxを目安にし、主要なCTAは高さ44pxを基準とする。

選択肢の見せ方は目的に応じて分ける。`Choice Button` は、`うっかりや`、`わんぱく`、`まじめ` のような横並びのプリセット選択に使い、`RadioButton` は説明を伴う単一選択の設定に使う。補足が必要でも画面を詰めたくない場合は `InfoButton` を使う。複数のボタンが同じ目的を持つ場合は、同じコンポーネント内でまとめて見せる。
Buttonに添える説明文は、Button群の直下に8px前後あけて置き、ボタンの意味を短く補足する。
...
親タグは一目で子タグの存在がわかるように、補助マークや説明ラベルを付けて表示する。子タグを持たないタグと見た目を揃えすぎず、選択の前に追加条件があることを明示する。

#### 8.1.1.1 初回利用設定画面

初回起動時タグ選択の画面内に、通知時刻・通知頻度・通知キャラクターの初期設定項目を続けて表示する。この画面では、選択済みタグの確認と通知設定を同じ画面内で行い、`次へ` で確認画面へ進む。タグ設定は `利用者コンテキスト`、`関心`、`選択済み` を区分して見せ、通知設定は別カードとして独立させる。`あなたの性格は？` と問いかける形で、通知の多さと性格プリセットを同時に伝える。後から開く `設定` シートとは別の、初回利用設定専用の導線として扱う。
`あなたの性格は？` の下には、ボタン群の意味を短く補足する説明文を置く。初回設定では説明を簡潔に保ち、後から開く通知設定シートよりも軽い情報量にする。

#### 8.1.1.2 初回利用設定確認画面

初回利用設定画面で選んだ内容を確認し、`保存して開始` を押した時点で初回利用設定を完了し、ホームへ進む。ここでの確認画面は、初回利用時に必要な導線であり、ホームの左側ドロワーから開く後からの `設定` / `通知設定` シートとは別物として扱う。

#### 8.1.2 ホームから履歴・設定へ遷移するメニュー

初回利用登録後のホーム画面では、履歴と設定への直接ボタンを並べず、ChatGPTのような左側スライドメニューを採用する。ホーム左上のメニューアイコンをタップすると、画面左からナビゲーションドロワーが滑り出し、`履歴` と `設定` を選べる。ドロワーには `TOKYO+` を表示し、ホーム画面本体には `TOKYO+` を表示しない。

- ドロワーはホームの上に重ねて表示する
- 背景は暗くして、ホームを一時的に操作不能にする
- 項目をタップすると、下からモーダルシートを表示する
- モーダルシート右上には `x` を置き、下方向スワイプでも閉じられるようにする
- モーダルシートを閉じても、ドロワーは表示されたままにする
- ホーム画面上のコンテンツ操作ではドロワーを閉じず、シートだけを差し替える
- 画面外タップ、戻る操作、閉じる操作でドロワーを閉じる
- ホーム内の主要判断カードは動かさない
- 履歴と設定は、ホームからの補助ナビゲーションとして扱う
- ホームの左上にはメニューアイコンを置き、右上の配置は使わない
...
ホーム画面では、タイトル、判断、`詳細を見る` / `閉じる`、divider、不確実性・注意点を常時表示する。参照データは独立した別コンポーネントとして切り離し、状態は画面の最下部に配置する。左上のメニューアイコンから左側ドロワーを開き、`履歴` と `設定` へ遷移する。展開状態では、同じ判断コンポーネントの中に詳細領域を差し込む。

`アドバイス詳細` は独立した遷移先画面としない。`詳細を見る` / `閉じる` の操作で、ホーム画面内の同じコンポーネントに詳細領域を割り込ませる。

### 8.3 設定画面

設定画面は、ホームの左側ドロワーから開く下部シートとして扱う。設定シート内の `通知設定` を押すと通知設定シートを開き、通知設定シートの `BackButton` から設定シートへ戻れるようにする。

後から開く通知設定シートでは、初回利用設定と同じ `あなたの性格は？` の選択肢を使う。ただし、説明量は初回設定より多くし、各選択肢の下に通知の多さ、間隔、1日の上限を補足として表示する。これにより、利用者が設定の意味を迷わず理解できる構成を優先する。
朝の通知時刻にも、毎朝の最初の通知時刻を決める説明文を添える。
`通知キャラクター` も同様に、話し方だけを変える設定だと分かる補足文を追加する。判断内容そのものは変わらず、表現のトーンだけを切り替える項目として扱う。
...
履歴画面は、ホームの左側ドロワーから開く下部シートとして扱う。
