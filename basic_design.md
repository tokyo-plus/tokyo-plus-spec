...
MVPでは、気象・地震情報は`tokyo`または`tokyo_wide`等の提供元の範囲、路上工事は`23ku`と道路区間、公衆トイレは`chuo`と個別地点、電車の運行情報はODPTで提供される事業者・路線スコープを対象とする。デモで特定地点のレコードを使う場合も、利用者設定ではなく検証用の固定条件として扱う。
...
公共交通オープンデータセンターの`odpt:TrainInformation` APIを利用し、対象データ範囲に含まれる路線の遅延、運休、ダイヤ乱れ等を検出する。`contextTags=["通勤"]`、`interestTags=["電車・運行情報"]`、`interestChildTags=["よく乗る路線:選択された路線ID"]`の場合、選択路線を優先して出発前に公式運行情報を確認する判断候補を作成する。
